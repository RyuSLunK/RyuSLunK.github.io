  ...
  .directive("canvas", function() {
    return {
        ...
        return {
          ...
          post: function(scope, elem, attrs, ctrl) {
            ...
            var onTouchPaint = function() {
              ...
              window.lastStroke = scope.context.stroke();
      ...
      controller: ['$scope', '$interval', function($scope, $interval) {
        window.canvasInterval = $interval;

        $scope.killz = function(){
          if ($scope.bleeders.length) {
            var totalHealth = 0;
            var bleedsRemaining = $scope.bleeders.length;
            var deadBleeds = [];
            console.log("working before bleed");
            $scope.bleeders.forEach(function(bleed) {
              bleed.health = bleed.health - 10;
              totalHealth = totalHealth + bleed.health;
              if (bleed.health <= 0) {
                deadBleeds.push(bleed);
              }
            });
            console.log("found " + deadBleeds.length + " dead bleeds");
            deadBleeds.forEach(function(dead) {
              $scope.bleeders.splice($scope.bleeders.indexOf(dead), 1);
            });
            if ($scope.bleeders.length == 0) {
              $interval.cancel($scope.death);
            } else {
              console.log("Bleeders: " + bleedsRemaining + "\nHealth: " + totalHealth);
            }

          }
        };
        //$scope.death = $interval(killz(), 1000);
        $scope.$watchCollection('bleeders', function(bleeds) {
          console.log(!$scope.death)
          if (!$scope.death){
            $scope.death = $interval(function(){$scope.killz()}, 1000);
          }

          console.log("bleeder count: " + $scope.bleeders.length);
        });
